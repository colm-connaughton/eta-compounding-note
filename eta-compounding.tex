...
\begin{align*}
\mathbb{E}\left[x_T \right] &=   T\,X_0\, \int_{-\infty}^\infty P(x, T)\,\left( z_1^x\,z_2^{1-x}\right)^T\\ 
& = T\,X_0\, \int_{-\infty}^\infty P(x, T)\,\mathrm{e}^{T\left(x\log z_1 + (1-x)\log z_2 \right)}\\
& = T^\frac{1}{2} X_0  \int_{-\infty}^\infty  \frac{d x}{\sqrt{2 \pi \,x\,(1-x)}}\,\mathrm{e}^{T\,\widetilde{\phi}(x)}\left(1 + \frac{1}{12}\left( 1-\frac{1}{x} - \frac{1}{1-x}\right) \frac{1}{T}   + \mathcal{O}\left(\frac{1}{T^2}\right)\right) \, \chi_{\left[0, 1\right]}(x),
\end{align*}
where 
\begin{align}
...
We can adapt the analysis of Redner \cite{redner1990random} to calculate the leading order behaviour for $0 \leq \eta \leq 1$. Write the random $\eta$-compounding random walk in its more general form as
\begin{align}
